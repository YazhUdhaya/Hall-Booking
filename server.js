    ...
    if (!(rooms[i].room_id === req.body.room_id)) {
      ...
      rooms[i].bookingDetails.forEach((book) => {
        if (
          ...
          //  return res.status(400).json({error:"Please select different time slot"})
        } else {
          result = 1;
      ...
      if (result) return res.status(200).json("Booking confirmed");
      else
        return res
          .status(400)
          .json({ error: "Please select different time slot" });
    }
